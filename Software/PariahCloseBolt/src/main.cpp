...
;  if (rpm > 50000)
  ...
  else if (firestate == idle) // if we're at idle send 0 throttle and reset some variables
  {
    ...
    if (!digitalRead(TRIG_PIN) && !alreadypressed)
    {
      ...
      maxrpm = 0; 
      triggertime = millis();
      boltstate = backward;    //start cycling the bolt
      boltstart= millis();
    }
    else if(digitalRead(TRIG_PIN))
    {
 ...
 throt = 0;
  ...
  else if (boltstate == backward) // move the bolt backwards quickly for a specific amount of time
  {
    if (boltstatetime() > 4)
    {
      int mappedinput = map(boltstatetime(),0,12,1650,1350);
    boltthrot = max(1350,mappedinput);
    }
  ...
  {
    boltthrot = 1350;
    if (boltstatetime() > 20) // once some time passes let go of the bolt, it'll stay backwards
    {
      //boltstate = boltIdle;
      boltstate = load;
    }
  }
  else if (boltstate == forwardhold) // apply low power for a bit to prevent bolt from bouncing forward
  {
    boltthrot = 250;
    if (boltstatetime() > 80) // once some time passes let go of the bolt, it'll stay backwards
    {
      //boltstate = boltIdle;
      boltstate = boltIdle;
    }
